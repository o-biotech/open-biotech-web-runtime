...
> = {
  async GET(req, ctx) {
    ...
    const eac: OpenBiotechEaC = eacSvc.Get(entLookup);
   
    if (eac.WarmQueries && queryLookup in eac.WarmQueries) 
      query = eac.WarmQuery[queryLookup]
...
export default function EaCWarmQuery({ Data }: PageProps<EaCWarmQueryPageData>) {
...
const postQueryData = async () => {
    ...
    if (primaryResult.length > 0)
      setDoesHaveResults(true);
    else
      setDoesHaveResults(false);
    }, 500);
};
  ...
  <label for="query" class="uppercase tracking-wide font-bold text-lg text-left">
    Query
  </label>
...
/>
        ...
        <small class="text-gray-500">Maximum 20 characters. Only lowercase letters, numbers, and dashes are allowed (no spaces).</small>
      </div>
